...
from app.gemini.vision import _build_genai_client
...
class GeminiEnricher:
    ...
    async def enrich(self, metadata: ResolvedBookMetadata) -> ResolvedBookMetadata:
        ...
        if not missing:
            logger.debug(
                "Todos los campos de enriquecimiento ya están presentes — se omite Gemini"
            )
            return metadata
        ...
        try:
            ...
            logger.warning(
                "Gemini enricher falló — el libro se creará sin enriquecimiento: %s",
                exc,
            )
            return metadata
    ...
    def _ensure_series_in_tagline(
        ...
        mentions_order = (
            metadata.order_to_read is not None
            and str(metadata.order_to_read) in tagline_lower
        )
        ...
        if metadata.order_to_read is not None:
            addition = f" Forma parte de la serie {metadata.series} y corresponde al libro {metadata.order_to_read}."
        else:
    ...
    def _should_enrich_synopsis(value: str | None) -> bool:
        ...
        return (
            any(pattern in lowered for pattern in weak_patterns) and len(cleaned) < 160
        )
